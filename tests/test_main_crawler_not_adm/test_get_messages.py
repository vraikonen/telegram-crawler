...
from modules.main_crawler import (get_entity, get_chat_info, get_messages, get_fwd_from, get_mentioned_chats)
...
async def test_get_chat_info(client_fixture, test_entity):
    async for client in client_fixture:
        ...
        desired_content = 'Another meme'
        desired_message_found = any(message.get('message') == desired_content for message in messages)
        assert desired_message_found is True

        # Check if there is a mentioned chat for which you are sure there is
            # not applicable for this channel cause there is no mentioned channels
        # desired_mention = 'test_mention'
        # desired_mention_found = any(mention == desired_mention for mention in mentioned_chats_full)
        ...
        # # Check fwd from, for example, we know that we fwd to our group from this username: test_channel_bastian
            # # not applicable for this channel cause there is no fwds
        # test_entity = 'test_fwd'
        # current_chat = await get_entity(client, test_entity)
